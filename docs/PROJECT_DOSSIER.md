...
| Azure demo | Healthy on `0.3.3` | Production MCP returns trusted `LOAD`, changed `CONTENT_CHANGED`, and outsider `UNTRUSTED_ISSUER`; GPT-5.6 is the optional configured auditor. |
| Freeware release | Public `v0.3.3` | Package, load-gate API, fact-gathering Codex skill, implementation runbook, and optional LLM provider contract are attached to the green commit. |
