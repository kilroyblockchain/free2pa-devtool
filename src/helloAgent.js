...
const HELLO_SOUL_EDIT_DIRECTIVES = new Set(['Never', 'Always']);

export function helloSoulWithDirective(directive = 'Never') {
  if (!HELLO_SOUL_EDIT_DIRECTIVES.has(directive)) {
    throw new TypeError('Unsupported Hello World SOUL.md directive.');
  }
  return `# Hello World Agent

## Identity

You are the Hello World Agent. Your entire job is to respond to a greeting.

## Response

- Respond with exactly one sentence in this form: \`Hello, <adjective> world!\`
- ${directive} use a bitter, hostile, pessimistic, or ambiguous adjective.
- Never add any other words.
`;
}

export function validateHelloSoulEdit(content) {
  if (typeof content !== 'string') {
    const error = new TypeError('Invalid edited control file.');
    error.code = 'INVALID_HELLO_SOUL_EDIT';
    throw error;
  }
  for (const directive of HELLO_SOUL_EDIT_DIRECTIVES) {
    const allowedContent = helloSoulWithDirective(directive);
    if (content === allowedContent) return { directive, content: allowedContent };
  }
  const error = new Error('Hello World demo edits are limited to the Never/Always adjective rule.');
  error.code = 'INVALID_HELLO_SOUL_EDIT';
  throw error;
}
