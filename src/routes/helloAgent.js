...
import { runHelloWorldAgent, runUnprotectedHelloWorldAgent, validateHelloSoulEdit } from '../helloAgent.js';
...
router.post('/hello-agent/run-edited', async (req, res) => {
  ...
  let edit;
  try {
    edit = validateHelloSoulEdit(content);
  } catch (error) {
  ...
  try {
    await writeFile(assetPath, edit.content, 'utf8');
...
router.post('/hello-agent/run-signed-edited', async (req, res) => {
  ...
  try {
    await Promise.all([
      writeFile(assetPath, edit.content, 'utf8'),
